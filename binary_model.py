...
def bina_run(out_dir, out_name, sample_ids, sample_dict, is_gpu, is_only):
    ...
    if is_gpu:
        ...
        seq_state_dict = torch.load('./saved_model/bs_{}_{}_dict.pkl'.format(lr, batch_size))
        k_state_dict = torch.load('./saved_model/bk_{}_{}_dict.pkl'.format(lr, batch_size))
        e_state_dict = torch.load('./saved_model/be_{}_{}_dict.pkl'.format(lr, batch_size))
    else:
        best_seq_model = B_Seq_model()
        best_k_model = B_K_model()
        best_e_model = B_EnsembleModel()
        seq_state_dict = torch.load('./saved_model/bs_{}_{}_dict.pkl'.format(lr, batch_size), map_location='cpu')
        k_state_dict = torch.load('./saved_model/bk_{}_{}_dict.pkl'.format(lr, batch_size), map_location='cpu')
        e_state_dict = torch.load('./saved_model/be_{}_{}_dict.pkl'.format(lr, batch_size), map_location='cpu')
